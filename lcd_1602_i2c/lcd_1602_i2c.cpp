...
int main() {
    ...
    // These lines will add strings to the binary visible using picotool
    // You can also set fields directly from your project’s CMake file
    bi_decl(bi_2pins_with_func(SDA_PIN, SCL_PIN, GPIO_FUNC_I2C));
    bi_decl(bi_program_description("LCD screen 1602."));
    ...
    while (true) {
        ...
        for (int m = 0; m < size; m += MAX_LINES) {
            for (int line = 0; line < MAX_LINES; line++) {
                int position = (MAX_CHARS / 2) - strlen(message[m + line]) / 2;
